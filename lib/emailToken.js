...
module.exports = (token, payload, callback) => {
    const mensaje = ['Éste es tu código personal para actualizar tu perfil en la olimpiada:', `${config.jwtRedeemURL(token)}`, ''];
    server.send({
        text: `${mensaje[0]}\n\n\n ${mensaje[1]} \n\n\n ${mensaje[2]}`,
        from: config.smtp_from,
        to: payload.email,
        'reply-to': config.smtp_reply_to,
        subject: 'Código para actualizar tu perfil en la olimpiada',
        attachment: [
